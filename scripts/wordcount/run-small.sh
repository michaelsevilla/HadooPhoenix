...
function start_oprofile(){
	...
	sudo opcontrol --event=CPU_CLK_UNHALTED:5000000000 --event=l1d:2000000 --event=l2_rqsts:200000 --event=l2_trans:2000 --event=dtlb_load_misses:2000000
...
do
	...
	INPUT_SIZE=$((${CHUNK}*${i}))
	NFILE=$((${i}-1))
	#if [ ${NFILE} -lt 10 ] ; then cat ${DATA}/part-0000${NFILE} >> ${DATA}/input; 
	#elif [ ${NFILE} -lt 100 ] ; then cat ${DATA}/part-000${NFILE} >> ${DATA}/input;
	#else cat ${DATA}/part-00${NFILE} >> ${DATA}/input;
	#fi
	...
	if [ ${i} -eq 200 ] ; then 
		...
		do
			...
			if [ $OPROFILE_ENABLED == 1 ] ; then 
				...
				opreport -l --image-path=/lib/modules/$(uname -r)/kernel 1>$OUTPUTDIR/symbols/symbols-iteration${i}.out 2>> $OUTPUTDIR/warnings.out
				opreport 1>$OUTPUTDIR/images/image-iteration${i}.out 2>> $OUTPUTDIR/warnings.out
			fi
