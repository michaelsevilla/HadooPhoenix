	...
	if [ $((${i}%10)) -eq 0 ] ; then 
		...
		do
			...
			if [ $FAULTOLERANCE_ENABLED == 1 ] ; then 
				...
				${PHOENIX_SORT}/${j} ${DATA}/input) 1>> $OUTPUTDIR/log.txt 2>> $OUTPUTDIR/timings/timing-${j}.out
			else
				echo "-- running benchmark..." >> $OUTPUTDIR/log.txt
				(time ${PHOENIX_SORT}/${j} ${DATA}/input) 1>> $OUTPUTDIR/log.txt 2>> $OUTPUTDIR/timings/timing-${j}.out
			fi
