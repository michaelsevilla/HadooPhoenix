...
# Date: 5/19/2013
#
# Script that times the storage transfer

function usage(){
        echo ""
        echo "Usage: ./run_storage-benchmark.sh <output directory>"
        echo ""
        echo "Example: "
        echo "   run-benchmark.sh ./deliverables "
	echo "*Ensure that: "
	echo "   - the hadoop directory is /mnt/vol1/msevilla/hadoop_src" 
...
do
	...
	if [ $((${i}%20)) -eq 0 ] ; then
		...
		(time ${HADOOP_DIR}/bin/hadoop dfs -copyToLocal ${DATA}/randomtextwriter-test ${LOCAL_DIR} \
		) 1>> $OUTPUTDIR/log.txt 2>> $OUTPUTDIR/timings/timing-${j}.out
		echo "" >> $OUTPUTDIR/timings/timing-${j}.out
		echo "Local data size: " `ls -alh ${LOCAL_DIR}` >> ${OUTPUTDIR}/log.txt
