...
# Delete the input
${HADOOP_DIR}/bin/hadoop dfs -rmr ${DATA}/randomwriter-test
${HADOOP_DIR}/bin/hadoop dfs -mkdir ${DATA}/randomwriter-test
...
do
	...
	if [ $((${i}%1)) -eq 0 ] ; then
	#if [ 1 -eq 1 ] ; then
		${HADOOP_DIR}/bin/hadoop dfs -rmr ${DATA}/randomwriter-output
		...
		echo "--------------------------------------------------" >> $OUTPUTDIR/warnings.out
		echo "Input size: " `${HADOOP_DIR}/bin/hadoop dfs -ls ${DATA}/randomwriter-test` >> $OUTPUTDIR/log.txt
		...
		(time ${HADOOP_DIR}/bin/hadoop jar ${HADOOP_DIR}/hadoop-examples-1.0.4.jar sort \
			${DATA}/randomwriter-test \
			${DATA}/randomwriter-output \
